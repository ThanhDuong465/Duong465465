...
if (args[0] == "all") {
    ...
    if (mention[0]) {
            ...
            return api.sendMessage(`🦋${(await Users.getData(mention[0])).name} đang đứng hạng ${rank} với tổng số tin nhắn là: ${infoUser.exp}`, event.threadID);
}
else {
            var { participantIDs } =(await Threads.getData(event.threadID)).threadInfo;
            //const countMess = (await Currencies.getData(event.senderID)).exp
            ...
            return api.sendMessage(`🧸Bạn đang đứng hạng ${rank} với tổng số tin nhắn là: ${infoUser.exp}`, event.threadID);
}
}
